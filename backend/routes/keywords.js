...
const { classifyKeyword } = require('../services/aiService');
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // 异步分类，不阻塞响应
    classifyKeyword(keyword.trim()).then(type => {
      db.updateKeyword(kw.id, { keyword_type: type });
      console.log(`[Keywords] "${keyword.trim()}" → type: ${type}`);
    }).catch(err => {
      console.error(`[Keywords] classifyKeyword error:`, err.message);
    });
